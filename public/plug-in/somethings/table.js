					...
					for (col in tdata.filters) {
						if (!hideRow) {
							...
							if (filter && col<cellsLength) {
								...
								/*add by Soul 2013-6-9 非操作过滤 begin*/					
								if (filter.regex && val.search) {
									var temp_reg = filter.toString().replace('/^', '').replace('/i', '');
									//if((temp_reg.length > 0 && $.trim(temp_reg) == '') || $.trim(temp_reg) == '!'){
									if(temp_reg.length > 0 && $.trim(temp_reg) == ''){
										//空
										hideRow = ($.trim(val)!= '');
									}
									else{
										...
										if(temp_reg != '!')
										{
											if(temp_reg.substr(0, 1) == '!'){
											//非
												//filter = eval('/^[^('+temp_reg.substr(1, temp_reg.length-1)+')]/i'); 
												filter = eval('/^'+temp_reg.substr(1, temp_reg.length-1)+'/i'); 
												hideRow=(val.search(filter)>-1);
											}
											else
											{
								...
								*/
								/*add by Soul 2013-6-9 非操作过滤 end*/
							}
						}
					}
				}
